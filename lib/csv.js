...
const CSV = (() => {
  const FIELDS = [
    'id', 'brand', 'brandUrls', 'code', 'discountValue', 'discountType',
    'expiryDate', 'minPurchase', 'terms', 'dateAdded', 'status',
  ];
  ...
  // Returns { toInsert: Coupon[], toUpdate: Coupon[], invalid: string[] }
  // Rows whose `code` matches an existing coupon are returned in toUpdate with the existing id
  // preserved so storage can overwrite them in-place. New codes go into toInsert.
  function importCoupons(csvText, existingCoupons) {
    const rows = parseCSV(csvText);
    if (rows.length < 2) return { toInsert: [], toUpdate: [], invalid: [] };

    const header = rows[0].map(h => h.trim());
    // Map code (lowercase) → existing coupon for fast lookup
    const existingByCode = new Map(existingCoupons.map(c => [c.code.toLowerCase(), c]));
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (!coupon.code) {
        invalid.push(`Row ${i + 1}: missing code`);
        continue;
      }

      const existing = existingByCode.get(coupon.code.toLowerCase());
      if (existing) {
        // Overwrite all fields but keep the existing id
        toUpdate.push({ ...coupon, id: existing.id });
      } else {
        existingByCode.set(coupon.code.toLowerCase(), coupon); // guard against dupes within the file
        toInsert.push(coupon);
      }
    }

    return { toInsert, toUpdate, invalid };
  }
